...
import { Star, Clock, CalendarDays, Play, Tv } from "lucide-react";
...
export async function generateMetadata({ params }: Props, parent: ResolvingMetadata): Promise<Metadata> {
    ...
    const series = await tmdb.getSeriesDetails(resolvedParams.id);
    const previousImages = (await parent).openGraph?.images || [];
    const bannerUrl = getImageUrl(series.backdrop_path || series.poster_path, "w1280");

    return {
        title: `${series.name} | Cineflix`,
        description: series.overview || "Descubre todo sobre esta serie en Cineflix V2.",
        openGraph: {
            title: `${series.name} | Cineflix`,
            description: series.overview || "Descubre todo sobre esta serie en Cineflix V2.",
...
export default async function SeriesDetailPage({ params }: Props) {
    ...
    const [series, session] = await Promise.all([
        tmdb.getSeriesDetails(resolvedParams.id),
    ...
    const runtime = series.episode_run_time?.[0]; // Average runtime per episode
    ...
    return (
        <div className="flex flex-col min-h-screen">
            ...
            <div className="relative w-full h-[60vh] md:h-[80vh]">
                <Image
                    src={getImageUrl(series.backdrop_path || series.poster_path, "w1920")}
                ...
                <div className="absolute bottom-0 left-0 w-full">
                    <div className="container mx-auto px-4 md:px-8 pb-12 flex flex-col md:flex-row gap-8 items-end">
                        ...
                        <AnimatedSection direction="up" delay={0.2} className="flex-1 space-y-4">
                            ...
                            {series.tagline && (
                                <p className="text-sky-300 italic text-lg drop-shadow">{series.tagline}</p>
                            )}

                            <div className="flex flex-wrap gap-4 pt-4">
                                {trailer ? (
                                    <VideoModal youtubeKey={trailer.key} title={`${series.name} Tráiler`} />
                                ) : null}
                ...
                {series.similar?.results && series.similar.results.length > 0 && (
                    <AnimatedSection direction="up" delay={0.3}>
                        <section>
                            ...
                            <Carousel>
                                {series.similar.results.map((sim: any) => (
                                    <div key={sim.id} className="w-[180px] md:w-[220px] flex-none">
                                        <MovieCard media={sim} mediaType="tv" />
                                    </div>
                                ))}
                            </Carousel>
                        </section>
